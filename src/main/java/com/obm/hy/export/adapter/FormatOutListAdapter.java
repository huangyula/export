...
public class FormatOutListAdapter extends BaseAdapter {

    private Context mContext;
    private List<FileFormatBean> mList;
    private boolean mShowLength=false;

    public FormatOutListAdapter(Context context, List<FileFormatBean> mList, boolean showlength) {
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        if (view == null) {
            hold = new HoldClass();
            view = LayoutInflater.from(mContext).inflate(R.layout.item_list_format_out, null);
        ...
        hold.et_nexportno.setText(fileFormat.getNexportno().toString());
        hold.et_nexportLen.setText(fileFormat.getNexportLen().toString());
        hold.ckb_bexportflag.setChecked(fileFormat.getBexportflag().equals("Y"));
                ...
                if (!hasFocus &&  (v != null) )
                {
                    ...
                    if (et_nexportno.getText().toString().trim().length()>0) {
                        if(mList.size()>0){
                            ...
                            fileFormat.setNexportno(Integer.valueOf(et_nexportno.getText().toString().trim()));
                        }
                    }
                }
            }
                ...
                if (!hasFocus &&  (v != null) )
                {
                    ...
                    if (et_nexportLen.getText().toString().trim().length()>0) {
                       if(mList.size()>0){
                           ...
                           fileFormat.setNexportLen(Integer.valueOf(et_nexportLen.getText().toString().trim()));
                       }
                    }
            ...
            public void onClick(View v) {
                if (v != null)
                {
                    if(mList.size()>0){
                        ...
                        fileFormat.setBexportflag(ckb_bexportflag.isChecked()?"Y":"N");
                    }
                }
            }
        });
